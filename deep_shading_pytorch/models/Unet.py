...
def down(input_channels, output_channels, groups, pooling = True, upsample = False):
    if pooling:
        if upsample:
...
def up(input_channels, output_channels, groups, upsample = True, deconv = False):
    if upsample:
        if deconv:
            return nn.Sequential(
                nn.Conv2d(input_channels, output_channels, groups = groups, kernel_size = 3, stride = 1, padding = 1),
                ...
                #nn.ReLU(inplace=True),
                nn.Upsample(mode='bilinear', scale_factor=2)
                #nn.ConvTranspose2d(output_channels, output_channels, groups = output_channels, kernel_size = 4, stride = 2, padding = (2,1), output_padding=(1,0))
                )
        else:
            return nn.Sequential(
                nn.Conv2d(input_channels, output_channels, groups = groups, kernel_size = 3, stride = 1, padding = 1),
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.down5 = down(128, 256, 16, True, True)
